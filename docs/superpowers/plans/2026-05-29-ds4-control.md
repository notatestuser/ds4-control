    ...
    @ViewBuilder private var cards: some View {
        if let s = metrics.currentSnapshot {
            MetricCardView(
                ...
                sparklineFixedRange: (0, 100), emphasized: true, gaugeFraction: s.memory.usagePercent / 100)  // emphasized before gaugeFraction (declared-property order for the synthesized memberwise init)
            HStack(spacing: 10) {
